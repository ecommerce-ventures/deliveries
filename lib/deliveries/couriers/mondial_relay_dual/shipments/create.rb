          ...
          class Create
            ...
            def execute
              builder = Nokogiri::XML::Builder.new(encoding: 'utf-8') do |xml|
                xml.ShipmentCreationRequest('xmlns:xsi0' => 'http://www.w3.org/2001/XMLSchema-instance', 'xmlns:xsd' => 'http://www.w3.org/2001/XMLSchema', 'xmlns' => 'http://www.example.org/Request') {
                  xml.Context {
                  ...
                  xml.ShipmentsList {
                    xml.Shipment {
                      ...
                      xml.DeliveryMode(Mode: params.dig(:delivery_mode, :mode), Location: params.dig(:delivery_mode, :location))
                      xml.CollectionMode(Mode: params.dig(:collection_mode, :mode), Location: params.dig(:collection_mode, :location))
                      xml.Parcels {
              ...
              unless shipment_number && pdf_url
                error_node = xml_doc.xpath('//ShipmentCreationResponse/StatusList/Status').find{ |n| %w{Critical\ Error Error}.include?(n[:Level]) }
